...
import (
	...
	omni "github.com/voxgig-sdk/answerbook-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AnswerbookError); is {
				out := map[string]any{
					"name":    "AnswerbookError",
...
func omniCtx(arg any, client *sdk.AnswerbookSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AnswerbookSDK); is {
			client = live
