...
func TestToolEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		toolRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.tool", setup.data)))
		var toolRef01Data map[string]any
		if len(toolRef01DataRaw) > 0 {
			toolRef01Data = core.ToMapAny(toolRef01DataRaw[0][1])
		}
...
func toolBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ANSWERBOOK_TEST_TOOL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ANSWERBOOK_TEST_TOOL_ENTID"])
