...
[MCP](https://modelcontextprotocol.io) server exposing the Answerbook SDK as
two agent tools — `answerbook_list` and `answerbook_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/answerbook-mcp)
make build

# 2. Provide credentials via the environment
export ANSWERBOOK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user answerbook \
  -- /absolute/path/to/answerbook-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./answerbook-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// answerbook_list: first page of records
{ "entity": "words_learning" }
{ "entity": "words_learning", "query": { } }

// answerbook_load: one record by id
{ "entity": "book_of_answer", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/answerbook-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user answerbook \
     -- "$PWD"/dist/*/answerbook-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `answerbook_list` and `answerbook_load` tools now appear
   in new sessions. Ask the agent to *"list words_learning using answerbook"*
   and it calls `answerbook_list` with `{"entity":"words_learning"}`.
...
```sh
export ANSWERBOOK_APIKEY=sk_live_xxx            # API key
export ANSWERBOOK_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `answerbook_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `answerbook_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ANSWERBOOK_APIKEY` | API key sent with every request. |
| `ANSWERBOOK_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

book_of_answer | get_api_doc | market_data | poetry__oracle | tool | word | words_learning

### Smoke test via HTTP (raw JSON-RPC)

```sh
./answerbook-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"answerbook_load","arguments":{"entity":"book_of_answer","query":{"id":1}}}}'
```
