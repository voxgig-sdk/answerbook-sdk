...
type Args struct {
	Entity string         `json:"entity" jsonschema:"book_of_answer | get_api_doc | market_data | poetry__oracle | tool | word | words_learning"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AnswerbookSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "answerbook_list",
		Description: "List records from Answerbook. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "answerbook_load",
		Description: "Load a single record from Answerbook. " +
...
func runOp(client *sdk.AnswerbookSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AnswerbookSDK, name string) (sdk.AnswerbookEntity, error) {
	switch strings.ToLower(name) {
	case "book_of_answer":
		return client.BookOfAnswer(nil), nil
	case "get_api_doc":
		return client.GetApiDoc(nil), nil
	case "market_data":
		return client.MarketData(nil), nil
	case "poetry__oracle":
		return client.PoetryOracle(nil), nil
	case "tool":
		return client.Tool(nil), nil
	case "word":
		return client.Word(nil), nil
	case "words_learning":
		return client.WordsLearning(nil), nil
