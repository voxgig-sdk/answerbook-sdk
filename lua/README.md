...
The Lua SDK for the Answerbook API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:BookOfAnswer(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Answerbook(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── answerbook_sdk.lua    -- Main SDK module
...
The main module (`answerbook_sdk`) exports the SDK constructor
