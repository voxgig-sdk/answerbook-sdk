...
local WordEntity = {}
WordEntity.__index = WordEntity


function WordEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WordEntity)
  self._name = "word"
  self._client = client
...
function WordEntity:get_name()
  return self._name
end


function WordEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WordEntity.new(self._client, opts)
end
...
function WordEntity:mark_deleted()
  self._deleted = true
end


function WordEntity:deleted()
  return true == self._deleted
end


function WordEntity:data_set(args)
  if args ~= nil then
...
function WordEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WordEntity:match_set(args)
  if args ~= nil then
...
function WordEntity:stream(action, args, callopts)
...
---@param reqmatch WordLoadMatch
---@param ctrl? table
---@return Word
---@return string? err
function WordEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WordEntity:_run_op(ctx, post_done)
  local utility = self._utility
