...
local PoetryOracleEntity = {}
PoetryOracleEntity.__index = PoetryOracleEntity


function PoetryOracleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PoetryOracleEntity)
  self._name = "poetry__oracle"
  self._client = client
...
function PoetryOracleEntity:get_name()
  return self._name
end


function PoetryOracleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PoetryOracleEntity.new(self._client, opts)
end
...
function PoetryOracleEntity:mark_deleted()
  self._deleted = true
end


function PoetryOracleEntity:deleted()
  return true == self._deleted
end


function PoetryOracleEntity:data_set(args)
  if args ~= nil then
...
function PoetryOracleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PoetryOracleEntity:match_set(args)
  if args ~= nil then
...
function PoetryOracleEntity:match_get()
...
function PoetryOracleEntity:stream(action, args, callopts)
...
---@param reqmatch PoetryOracleLoadMatch
---@param ctrl? table
---@return PoetryOracle
---@return string? err
function PoetryOracleEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PoetryOracleEntity:_run_op(ctx, post_done)
  local utility = self._utility
