...
local WordsLearningEntity = {}
WordsLearningEntity.__index = WordsLearningEntity


function WordsLearningEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WordsLearningEntity)
  self._name = "words_learning"
  self._client = client
...
function WordsLearningEntity:get_name()
  return self._name
end


function WordsLearningEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WordsLearningEntity.new(self._client, opts)
end
...
function WordsLearningEntity:mark_deleted()
  self._deleted = true
end


function WordsLearningEntity:deleted()
  return true == self._deleted
end


function WordsLearningEntity:data_set(args)
  if args ~= nil then
...
function WordsLearningEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WordsLearningEntity:match_set(args)
  if args ~= nil then
...
function WordsLearningEntity:match_get()
...
function WordsLearningEntity:stream(action, args, callopts)
...
function WordsLearningEntity:_run_op(ctx, post_done)
  local utility = self._utility
