...
local GetApiDocEntity = {}
GetApiDocEntity.__index = GetApiDocEntity


function GetApiDocEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetApiDocEntity)
  self._name = "get_api_doc"
  self._client = client
...
function GetApiDocEntity:get_name()
  return self._name
end


function GetApiDocEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetApiDocEntity.new(self._client, opts)
end
...
function GetApiDocEntity:mark_deleted()
  self._deleted = true
end


function GetApiDocEntity:deleted()
  return true == self._deleted
end


function GetApiDocEntity:data_set(args)
  if args ~= nil then
...
function GetApiDocEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetApiDocEntity:match_set(args)
  if args ~= nil then
...
function GetApiDocEntity:match_get()
...
function GetApiDocEntity:stream(action, args, callopts)
...
---@param reqmatch GetApiDocLoadMatch
---@param ctrl? table
---@return GetApiDoc
---@return string? err
function GetApiDocEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GetApiDocEntity:_run_op(ctx, post_done)
  local utility = self._utility
