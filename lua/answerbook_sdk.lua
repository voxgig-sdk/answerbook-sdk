...
-- by passing it.
function AnswerbookSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AnswerbookSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AnswerbookSDK: graphql: " .. msg
    res.graphql = errors
  end
