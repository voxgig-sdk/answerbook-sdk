...
module AnswerbookUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AnswerbookResponse.new({})
    ...
    if fetch_err
      ...
      response = AnswerbookResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AnswerbookResponse.new(fetched)
    else
