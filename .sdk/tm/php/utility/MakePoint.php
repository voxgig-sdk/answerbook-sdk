...
// Answerbook SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class AnswerbookMakePoint
{
    public static function call(AnswerbookContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof AnswerbookError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = AnswerbookHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = AnswerbookHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
