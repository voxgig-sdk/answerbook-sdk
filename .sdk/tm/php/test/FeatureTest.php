...
class FtCtrl extends AnswerbookControl
{
...
class FtClient
{
    ...
    private AnswerbookUtility $utility;

    public function __construct(AnswerbookUtility $utility, array $options)
    {
    ...
    public function get_utility(): AnswerbookUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AnswerbookContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AnswerbookContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AnswerbookContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AnswerbookConfig::make_feature($name);
    ...
    private static function buildUrl(AnswerbookSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AnswerbookError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AnswerbookResponse($fetched);
        }
    ...
    private function fail(AnswerbookContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AnswerbookContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
