// Command Answerbook-cli is a CLI + REPL for the Answerbook SDK,
...
const prompt = "answerbook"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "book_of_answer get_api_doc market_data poetry__oracle tool word words_learning"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ANSWERBOOK_APIKEY (api key), ANSWERBOOK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ANSWERBOOK_APIKEY carries the API key and
	// ANSWERBOOK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ANSWERBOOK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ANSWERBOOK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAnswerbookSDK(opts)
