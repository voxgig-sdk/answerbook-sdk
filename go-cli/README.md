...
boru-driven command-line client **and** interactive REPL for the Answerbook
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/answerbook-cli)
make build
...
./answerbook-cli load 1 book_of_answer            # {id:1} shorthand
./answerbook-cli load '{id:1}' book_of_answer       # explicit match map

# 5. Override the API base URL for a single call
ANSWERBOOK_BASE=https://api.example.com ./answerbook-cli load 1 book_of_answer

# 6. No arguments -> interactive REPL
./answerbook-cli
answerbook> load 1 book_of_answer
answerbook> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/answerbook-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/answerbook-cli load 1 book_of_answer
   ```
...
```sh
./answerbook-cli load 1 book_of_answer          # scalar shorthand for {id:1}
./answerbook-cli load '{id:1}' book_of_answer     # explicit match map
```
...
./answerbook-cli load 1 book_of_answer
```
...
Run with no arguments to open a REPL (prompt `answerbook>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `book_of_answer`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/answerbook-cli`. |
...
(`./answerbook-cli <expr>`) and interactively (the REPL), and expressions compose the
