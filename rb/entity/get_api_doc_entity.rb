...
class GetApiDocEntity
  ...
  def make
    opts = @_entopts.dup
    GetApiDocEntity.new(@_client, opts)
  end
  ...
  # @return [GetApiDoc, Hash] the current GetApiDoc data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of GetApiDoc fields)
  def match_get
  ...
  #
  # @param reqmatch [GetApiDocLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetApiDoc.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetApiDoc, Hash] the loaded GetApiDoc; raises AnswerbookError on failure
