...
class WordEntity
  ...
  # @return [Word, Hash] the current Word data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Word fields)
  def match_get
  ...
  # Load a single Word.
  #
  # @param reqmatch [WordLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Word.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Word, Hash] the loaded Word; raises AnswerbookError on failure
