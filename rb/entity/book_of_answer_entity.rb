...
class BookOfAnswerEntity
  ...
  def make
    opts = @_entopts.dup
    BookOfAnswerEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AnswerbookHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [BookOfAnswer, Hash] the current BookOfAnswer data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AnswerbookHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of BookOfAnswer fields)
  def match_get
  ...
  #
  # @param reqmatch [BookOfAnswerLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.BookOfAnswer.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [BookOfAnswer, Hash] the loaded BookOfAnswer; raises AnswerbookError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AnswerbookHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
