...
class WordsLearningDirectTest < Minitest::Test
  def test_direct_list_words_learning
    setup = words_learning_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-words_learning", setup[:live] ? "live" : "unit")
    if _should_skip
...
def words_learning_direct_setup(mockres)
