...
class WordsLearningEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = AnswerbookSDK.test(seed, nil)
    seen = base.WordsLearning(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = AnswerbookConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = AnswerbookSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
      sdk.WordsLearning(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = words_learning_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "words_learning." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ANSWERBOOK_TEST_WORDS_LEARNING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    words_learning_ref01_data = nil
    if words_learning_ref01_data_raw.length > 0
...
def words_learning_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "words_learning", "WordsLearningTestData.json")
  ...
  entid_env_raw = ENV["ANSWERBOOK_TEST_WORDS_LEARNING_ENTID"]
