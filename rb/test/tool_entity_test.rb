...
class ToolEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ANSWERBOOK_TEST_TOOL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    tool_ref01_data = nil
    if tool_ref01_data_raw.length > 0
...
def tool_basic_setup(extra)
  ...
  entid_env_raw = ENV["ANSWERBOOK_TEST_TOOL_ENTID"]
