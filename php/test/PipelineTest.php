...
{
    private static function utility(): AnswerbookUtility
    {
        return new AnswerbookUtility();
    }
    ...
    private static function ctx(array $over = []): AnswerbookContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AnswerbookContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AnswerbookOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AnswerbookError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AnswerbookOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AnswerbookMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AnswerbookOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AnswerbookMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AnswerbookOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AnswerbookMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AnswerbookSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AnswerbookMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AnswerbookResponse([]),
            'result' => new AnswerbookResult([]),
        ]);
        [, $err] = AnswerbookMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AnswerbookSpec([]),
            'response' => null,
            'result' => new AnswerbookResult([]),
        ]);
        [, $err] = AnswerbookMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AnswerbookSpec(['step' => 's']),
            'response' => new AnswerbookResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AnswerbookResult([]),
        ]);
        [, $err] = AnswerbookMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AnswerbookSpec(['step' => 's']),
            'response' => new AnswerbookResponse(self::resp(200, ['v' => 1])),
            'result' => new AnswerbookResult([]),
        ]);
        [, $err] = AnswerbookMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AnswerbookSpec(['step' => 's']),
            'response' => new AnswerbookResponse(self::resp(200, ['v' => 2])),
            'result' => new AnswerbookResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AnswerbookMakeResponse::call($ctx);
    ...
    {
        $preset = new AnswerbookResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AnswerbookSpec([]),
            'response' => new AnswerbookResponse([]),
            'result' => new AnswerbookResult([]),
        ...
        [$got, $err] = AnswerbookMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AnswerbookResult([])]);
        [, $err] = AnswerbookMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AnswerbookSpec([]), 'result' => null]);
        [, $err] = AnswerbookMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AnswerbookSpec(['step' => 's']),
            'result' => new AnswerbookResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AnswerbookOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AnswerbookMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AnswerbookSpec(['step' => 's']),
            'result' => new AnswerbookResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AnswerbookOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AnswerbookMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AnswerbookResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AnswerbookSpec([]),
            'result' => new AnswerbookResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AnswerbookMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AnswerbookMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AnswerbookError('boom', 'boom');
        $utility->fetcher = function (AnswerbookContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AnswerbookSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AnswerbookMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AnswerbookContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AnswerbookMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AnswerbookResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AnswerbookMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AnswerbookContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AnswerbookSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AnswerbookMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AnswerbookResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AnswerbookSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AnswerbookMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AnswerbookMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AnswerbookMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AnswerbookResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AnswerbookDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AnswerbookResult(['ok' => false])]);
        $this->expectException(AnswerbookError::class);
        AnswerbookDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AnswerbookResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AnswerbookMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AnswerbookMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AnswerbookBaseFeature();
        $b = new AnswerbookBaseFeature();
        AnswerbookFeatureAdd::call($ctx, $a);
        AnswerbookFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AnswerbookBaseFeature
    {
        $f = new AnswerbookBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AnswerbookFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AnswerbookContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AnswerbookSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AnswerbookPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        AnswerbookPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AnswerbookResponse(['headers' => null]),
            'result' => new AnswerbookResult([]),
        ]);
        AnswerbookResultHeaders::call($ctx);
