...
class PoetryOracleEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = AnswerbookSDK::test(null, null);
        $ent = $testsdk->PoetryOracle(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = poetry__oracle_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "poetry__oracle." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $poetry__oracle_ref01_data = null;
        if (count($poetry__oracle_ref01_data_raw) > 0) {
...
function poetry__oracle_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/poetry__oracle/PoetryOracleTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["poetry__oracle01", "poetry__oracle02", "poetry__oracle03"] as $k) {
