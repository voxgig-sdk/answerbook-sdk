...
class AnswerbookNetsimFeature extends AnswerbookBaseFeature
{
    ...
    private function _simulate(AnswerbookContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(AnswerbookContext $ctx, array $applied): void
    {
        $client = $this->client;
