...
class AnswerbookMakeError
{
    public static function call(?AnswerbookContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AnswerbookContext([], null);
        }
        $op = $ctx->op ?? new AnswerbookOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AnswerbookResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AnswerbookError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AnswerbookError('', $msg, $ctx);
        ...
        if ($err instanceof AnswerbookError) {
