...
//     (\`new AnswerbookSDK(...)\` / \`AnswerbookSDK.test(...)\` become
//     \`AnswerbookSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"book_of_answer":{"test01":{"id":"test01"}},"get_api_doc":{"test01":{"id":"test01"}},"market_data":{"test01":{"id":"test01"}},"poetry__oracle":{"test01":{"id":"test01"}},"tool":{"test01":{"id":"test01"}},"word":{"test01":{"id":"test01"}},"words_learning":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AnswerbookSDK(...)\` and any
// \`AnswerbookSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AnswerbookSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AnswerbookSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
