...
describe('GetApiDocEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'get_api_doc.' + op, live)) return
    }
    ...
    let get_api_doc_ref01_data = Object.values(setup.data.existing.get_api_doc)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/get_api_doc/GetApiDocTestData.json')
