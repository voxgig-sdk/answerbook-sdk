...
describe('WordDirect', async () => {
  ...
  test('direct-load-word', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-word', setup.live)) return
    if (skipIfMissingIds(t, setup, ["category01","word01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ANSWERBOOK_TEST_WORD_ENTID']
