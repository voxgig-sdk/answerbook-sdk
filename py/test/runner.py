...
class AnswerbookTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AnswerbookTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AnswerbookTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AnswerbookTestRunner.getenv("ANSWERBOOK_TEST_LIVE")
        override = AnswerbookTestRunner.getenv("ANSWERBOOK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AnswerbookTestRunner.getenv(key)
        ...
        explain = AnswerbookTestRunner.getenv("ANSWERBOOK_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ANSWERBOOK_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AnswerbookTestRunner._test_control is not None:
            return AnswerbookTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AnswerbookTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AnswerbookTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AnswerbookTestRunner.load_test_control()
...
def load_env_local():
    AnswerbookTestRunner.load_env_local()


def env_override(m):
    return AnswerbookTestRunner.env_override(m)


def entity_data(v):
    return AnswerbookTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AnswerbookTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AnswerbookTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AnswerbookTestRunner.load_test_control()


def live_client_options():
    return AnswerbookTestRunner.live_client_options()


def live_delay_ms():
    return AnswerbookTestRunner.live_delay_ms()
