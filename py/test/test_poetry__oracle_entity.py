...
class TestPoetryOracleEntity:

    def test_should_create_instance(self):
        testsdk = AnswerbookSDK.test(None, None)
        ent = testsdk.PoetryOracle(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _poetry__oracle_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "poetry__oracle." + _op, "live" if _live else "unit")
            if _skip:
        ...
        poetry__oracle_ref01_data = None
        if len(poetry__oracle_ref01_data_raw) > 0:
...
def _poetry__oracle_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/poetry__oracle/PoetryOracleTestData.json")
