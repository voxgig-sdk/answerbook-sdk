...
from answerbook_sdk.answerbook_types import (
    PoetryOracle,
...
class PoetryOracleEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PoetryOracleEntity(self._client, opts)
    ...
    def data_get(self) -> PoetryOracle:
    ...
    def match_get(self) -> PoetryOracle:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PoetryOracle:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PoetryOracle().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
